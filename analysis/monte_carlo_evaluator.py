...
def run_simulations(
    ...
    env = CricketEnv(
        total_balls=total_balls, match_format=match_format,
        target=target, batter_skill=batter_skill,
    )
    ...
    for i in range(1, n_sims + 1):
        ...
        if verbose_every and i % verbose_every == 0:
            ...
            print(f"  [{i:>6,}/{n_sims:,}]  Avg score (last {verbose_every}): {avg_score:.1f}  "
...
def compute_summary(results: Dict) -> Dict:
    ...
    summary = {
        ...
        "top_delivery":       max(results["delivery_counts"], key=results["delivery_counts"].get)
                              if results["delivery_counts"] else "N/A",
    }
...
def plot_score_histogram(results: Dict, save_path: Path) -> None:
    ...
    ax.axvline(np.mean(scores),   color="#ef4444", lw=2, linestyle="--", label=f"Mean: {np.mean(scores):.1f}")
    ax.axvline(np.median(scores), color="#f59e0b", lw=2, linestyle=":",  label=f"Median: {int(np.median(scores))}")
    ax.set_xlabel("Total Score",  fontsize=12)
    ax.set_ylabel("Frequency",    fontsize=12)
    ax.set_title(f"Score Distribution — {results['match_format']} ({results['n_sims']:,} simulations)", fontsize=13)
...
def plot_shot_heatmap(results: Dict, save_path: Path) -> None:
    ...
    ax.set_title(f"Shot Selection Heatmap — {results['match_format']} ({results['n_sims']:,} sims)", fontsize=12)
    ax.set_xlabel("Shot"); ax.set_ylabel("Delivery")
...
def plot_win_probability(match_format: str, agent, save_path: Path,
                          skill: float = 0.5) -> None:
    ...
    for target in targets:
        env = CricketEnv(total_balls=total_balls, match_format=match_format,
                          target=target, batter_skill=skill)
        wins = 0
...
def plot_wicket_distribution(results: Dict, save_path: Path) -> None:
    ...
    ax.set_ylabel("Frequency (%)", fontsize=12)
    ax.set_title(f"Wickets per Innings — {results['match_format']} ({results['n_sims']:,} sims)", fontsize=13)
...
def _parse():
    ...
    p.add_argument("--no-win-curve", action="store_true",
...
if __name__ == "__main__":
    ...
    results = run_simulations(
        agent, args.format, args.sims,
        target=args.target, batter_skill=args.skill,
    )
    ...
    if not args.no_win_curve:
        plot_win_probability(args.format, agent, PLOT_DIR / "mc_win_probability.png",
                             skill=args.skill)

    save_results(results, summary, RESULTS_DIR / f"mc_results_{args.format.lower()}.json")
