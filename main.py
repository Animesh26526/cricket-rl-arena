...
class HumanMatch:
    ...
    def __init__(self, overs: int, home_team: Team, away_team: Team,
                 batter_agent=None, bowler_agent=None,
    ...
    def _select_bowler(self) -> None:
        if self.bowling_team == self._ai_team and self.bowler_agent is not None:
            ...
            self.bowling_team.current_bowler = next(
                (p for p in self.bowling_team.players if p.name == self.bowling_team.current_bowler_name),
                None
            )
        ...
        for i, name in enumerate(options, 1):
            print(f"  {i}. {name}")
        choice = get_int_input(f"Choose Bowler (1-{len(options)}): ", 1, len(options)) - 1
        self.bowling_team.current_bowler_name = options[choice]
        # Find existing player from roster instead of creating new one
        self.bowling_team.current_bowler = next(
            (p for p in self.bowling_team.players if p.name == self.bowling_team.current_bowler_name),
    ...
    def _compute_batter_state(self) -> List[float]:
        ...
        runs_required     = float(self.target - self.batting_team.score) if self.target is not None else 0.0
        overs_played      = self.balls_bowled / 6.0 if self.balls_bowled > 0 else 0.0
        current_rr        = min(36.0, self.batting_team.score / overs_played) if overs_played > 0 else 0.0
        required_rr       = 0.0
    ...
    def _select_delivery(self) -> None:
        ...
        for i, d in enumerate(deliveries, 1):
            print(f"  {i}. {d}")
        choice = get_int_input(f"Choose Delivery (1-{len(deliveries)}): ", 1, len(deliveries)) - 1
    ...
    def _bowl_ball(self, wickets_disabled: bool = False) -> None:
        ...
        if extra != "None":
            ...
            self.result = self._engine.sample_outcome(
                ...
                self.match_format, self.powerplay,
            )
        ...
        while self.balls_bowled < self.total_balls:
            ...
            while self.balls_bowled < over_start + 6:
                ...
                if self.batting_team.wickets == 10:
                    print("All Out!")
                    return
                if self.innings in (2, 4) and self.target and self.batting_team.score >= self.target:
    ...
    def _man_of_the_match(self) -> None:
        ...
        if best:
            ...
            print(f"   Batting: {best.runs} off {best.balls}  |  Bowling: {best.wickets}W / {best.runs_conceded}R")
        else:
    ...
    def play_match(self, match_format: str = "T20") -> None:
        ...
        print(f"{ht.name}: {ht.score}/{ht.wickets}  vs  {at.name}: {at.score}/{at.wickets}")
    ...
    def _play_super_over(self) -> None:
        ...
        input(f"Press Enter for {self.batting_team.name} Super Over (target {self.target})...")
        self.play_innings()
...
def setup_team(team_name: str) -> Team:
    ...
    if captain not in player_names:
        print(f"⚠ Captain '{captain}' not in players list! Setting captain to first player.")
        team.captain = player_names[0]

    fast_bowlers = set()
    spin_bowlers = set()
    
    fast_count = get_int_input(f"\nHow many fast bowlers for {team_name}? (1-6): ", 1, 6)
    ...
    for i in range(spin_count):
        while True:
            ...
            if bowler_name not in player_names:
                ...
                print(f"  ⚠ '{bowler_name}' is already a fast bowler. Try another player.")
            else:
...
if __name__ == "__main__":
    ...
    mode = get_str_input("\nGame Mode (Human vs Human / Human vs AI): ", ["Human vs Human", "Human vs AI"])
    ...
    if is_ai_mode:
        ai_team_choice = get_str_input(
            f"Which team will be AI? ({t1_name} / {t2_name}): ",
            [t1_name, t2_name]
        )
