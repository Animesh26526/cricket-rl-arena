...
class MultiAgentCricketEnv:
    ...
    def reset(self) -> Tuple[List[float], List[float]]:
        ...
        self.available_shots = self._engine.get_available_shots(
            self.delivery, self.stumps
        )
    ...
    def get_batter_state(self) -> List[float]:
        ...
        rrr = (
            min(36.0, runs_req / overs_remaining)
            if (overs_remaining > 0 and self.target)
            else 0.0
        )
        return [
            float(delivery_idx),
    ...
    def get_bowler_state(self) -> List[float]:
        ...
        """
        economy = (
            min(36.0, self.score / (self.balls_bowled / 6.0))
    ...
    def _apply_result(self, result: str) -> float:
        ...
        if runs == 4:
            reward += REWARD_FOUR_BONUS
        if runs == 6:
            reward += REWARD_SIX_BONUS
    ...
    def _update_pp(self):
        if self.powerplay and self.balls_bowled // 6 >= self._pp_end.get(
            self.match_format, 0
        ):
