...
class ProbabilityEngine:
    ...
    def get_matrix(
        self, match_format: str, powerplay: bool, skill: float = 0.5
    ) -> Dict:
        ...
        for shot in ALL_SHOTS:
            for delivery in ALL_DELIVERIES:
                for stumps in ("Touching", "Not Touching"):
                    ...
                    dismissals = self._get_dismissals(
                        shot, delivery, stumps, match_format
                    )
    ...
    def _get_dismissals(self, shot, delivery, stumps, match_format) -> OutcomeList:
        if shot == "Leave":
            if stumps == "Touching":
                return self._scale(
                    [("Bowled", 60), ("L.B.W", 20), ("Edged And Caught Behind", 5)],
                    match_format,
                )
            return []
        ...
        if shot == "Pull" and delivery == "Bouncer":
            return self._scale(
                [("Caught", 20), ("Edged And Caught Behind", 5), ("Run Out", 10)],
                match_format,
            )
        if shot in ("Cut", "Square Cut") and delivery in (
            "Off Cutter",
            "Top Spin",
            "Flipper",
        ):
            return self._scale(
                [("Caught", 20), ("Edged And Caught Behind", 10), ("Run Out", 10)],
                match_format,
            )
        if shot == "Sweep Shot" and delivery in ("Top Spin", "Leg Spin"):
            return self._scale(
                [
                    ...
                    ("Caught", 10),
                    ("Run Out", 10),
                    ("Edged And Caught Behind", 5),
                ],
                match_format,
            )
        if shot in ("Straight Drive", "On Drive") and delivery in (
            "Straight",
            "Arm Ball",
            "Leg Cutter",
        ):
            return self._scale(
                [("L.B.W", 5), ("Caught", 5), ("Run Out", 10)], match_format
            )
        if shot in ("Off Drive", "Cover Drive") and delivery in (
            "In Swing",
            "Out Swing",
            "Off Cutter",
        ):
            return self._scale(
                [("Caught", 10), ("Run Out", 10), ("Edged And Caught Behind", 5)],
                match_format,
            )
        return self._scale(
            [
